...
const FeatureTab = (
  ...
  return (
    ...
    >
      ...
      <div
        className={'size-12 border border-muted rounded-lg inline-flex items-center justify-center'}
      >
        <DotLottiePlayer src={props.icon} className={'size-5'} autoplay ref={dotLottieRef} />
      </div>
      <div className={'font-medium'}>{props.title}</div>
      {props.isNew && (
        <div className={'text-xs rounded-full text-white px-2 py-0.5 bg-[#1e8a8a] font-semibold'}>
          New
        </div>
      )}
    </div>
